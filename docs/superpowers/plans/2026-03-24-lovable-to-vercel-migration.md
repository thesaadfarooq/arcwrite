...
Also rename `SUPABASE_PUBLISHABLE_KEY` to `SUPABASE_ANON_KEY` for clarity (standard Supabase naming):
...
- [ ] **Step 3: Update .env variable name**

Rename `VITE_SUPABASE_PUBLISHABLE_KEY` to `VITE_SUPABASE_ANON_KEY` in `.env`:
...
- [ ] **Step 4: Find and replace any other references to the old env var name**

Run: `grep -r "PUBLISHABLE_KEY" src/`

Update every hit to use `VITE_SUPABASE_ANON_KEY` instead. The known location is:
- `src/lib/story-api.ts` line 38: `Authorization: \`Bearer ${import.meta.env.VITE_SUPABASE_PUBLISHABLE_KEY}\``

This line will be removed entirely in Task 8 when we rewrite story-api.ts to call Vercel routes instead. But if the build needs to pass before that, update it now.
...
| `VITE_SUPABASE_ANON_KEY` | Vercel env | Client-side Supabase anon key |
| `SUPABASE_URL` | Vercel env | Server-side Supabase URL (API routes) |
| `SUPABASE_ANON_KEY` | Vercel env | Server-side Supabase anon key (API routes) |
| `SUPABASE_SERVICE_ROLE_KEY` | Vercel env | Server-side admin access (export-story) |
| `OPENAI_API_KEY` | Vercel env | AI generation |
| `STRIPE_SECRET_KEY` | Vercel env | Stripe operations |
| `STRIPE_WEBHOOK_SECRET` | Vercel env | Stripe webhook verification (if used) |
