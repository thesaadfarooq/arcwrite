...
export default function StoryNew() {
  ...
  const handleStart = () => {
    // For now, navigate to a demo story. Later, this will call AI and create a DB record.
    const params = new URLSearchParams();
    if (premise) params.set("premise", premise);
    if (selectedGenre) params.set("genre", selectedGenre);
    if (selectedTone) params.set("tone", selectedTone);
    navigate(`/story/demo?${params.toString()}`);
  };
      ...
      <main className="pt-24 pb-16 px-6 max-w-2xl mx-auto">
        ...
        {step === "genre" && (
          <div className="animate-fade-up">
            ...
            <div className="flex justify-end gap-3 mt-6">
              <Button variant="outline" onClick={() => { setStep("premise"); }}>
                Add a premise
              </Button>
              <Button variant="outline" onClick={() => setStep("tone")} disabled={!canProceed}>
                Choose tone <ArrowRight className="w-4 h-4 ml-1" />
        ...
        {step === "surprise" && (
          <div className="animate-fade-up text-center">
            ...
            <p className="text-muted-foreground mb-8 max-w-md mx-auto">
              We'll generate a random premise and dive straight in. No planning, pure discovery.
            </p>
            <div className="flex justify-center gap-3">
              ...
              <Button onClick={handleStart}>
                <Sparkles className="w-4 h-4 mr-1" /> Surprise me
              </Button>
            </div>
          </div>
        )}
        ...
        {step === "tone" && (
          <div className="animate-fade-up">
            ...
            <div className="flex justify-end gap-3 mt-6">
              ...
              <Button onClick={handleStart} disabled={!selectedTone}>
                Begin writing <ArrowRight className="w-4 h-4 ml-1" />
              </Button>
            </div>
          </div>
        )}
